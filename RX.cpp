  ...
  //Variabili necessarie RC!
  int16_t rcData[RC_CHANS];          // interval [1000;2000]
  int16_t rcSerial[8];               // interval [1000;2000] - is rcData coming from MSP
  //static int16_t rcCommand[4];     // interval [1000;2000] for THROTTLE and [-500;+500] for ROLL/PITCH/YAW
  uint8_t rcSerialCount = 0;         // a counter to select legacy RX when there is no more MSP rc serial data
  unsigned long lastRCCommand = 0;   // CACCIA: safety - if arduino does not receive commands from the receiver
  bool flgSendCommand = false;       // CACCIA: it allows arduino to send different commands to pixhawk
  ...
  /**************************************************************************************/
    ...
    ISR(RX_PC_INTERRUPT) { //this ISR is common to every receiver channel, it is call everytime a change state occurs on a RX input pin
      ...
      lastRCCommand = millis(); // CACCIA: safety - if arduino does not receive commands from the receiver
      flgSendCommand = true;    // CACCIA: if I receive different sticks and buttons values then arduino it is allowed to modify the signal to the pixhawk
